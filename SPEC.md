...
| US7 | **作为涂装玩家，我希望在 3D 预览中截图取色**，因为 WebGL Canvas 本身不能取色。我调好光后截图，在截图上取高光区域的象牙白色，再送入混色引擎算配方。 | 点截图按钮 → 当前帧转为图片 → 传入取色组件 → eyedropper 取色 → 可送混色引擎。 |
...
- 3D 预览页"截图取色"按钮
- `renderer.domElement.toDataURL()` 截取当前帧（需 `preserveDrawingBuffer: true`）
- 截图传入取色组件 → eyedropper 取色
- 取到的 RGB 可送入混色引擎或直接入库
...
- **流程 B — 光影参考（涂装中）**：3D 预览 → 加载几何体/STL → 色轮实时染色 → 调材质 + 多光源 → 截图取色 → 混色引擎 → 保存打光方案
...
- [ ] `preserveDrawingBuffer: true` 下截图取色可用
