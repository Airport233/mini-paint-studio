...
**定位**：色轮工具页面下的一个 Tab。帮助用户在矩形色谱上自定义色域范围，在该范围内取色。

**功能**：
- 矩形色谱（非环形色轮）覆盖 sRGB 颜色空间
- 用户在色谱上放置 3 个或 4 个锚点，连线形成三角形或四边形区域
- 区域内部渐变填充，表示该色域内可混合出的颜色范围
- 取色器可在区域内或边缘上取色 → RGB 送入混色引擎（与色轮取色相同流程）

**交互流程**：
色谱上放置锚点 → 连成区域 → 区域内取色 → 送入混色引擎计算配方

**与混色引擎的关系**：色域工具负责"在哪个颜色空间区域内探索"，混色引擎负责"这个具体颜色用你的库漆怎么混出来"。
